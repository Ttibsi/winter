...
constexpr int test_parseExpression([[maybe_unused]] Willow::Test* test) {
    ...
    if (node->op->type != Winter::TokenType::PLUS) {
        return 4;
    }
...
constexpr int test_parseFunc([[maybe_unused]] Willow::Test* test) {
    ...
    if (node->params.at(0).type != Winter::TokenType::IDENT) {
        return 5;
    }
    if (node->params.at(0).len != 4) {
        return 6;
    }
...
constexpr int test_parse_tree([[maybe_unused]] Willow::Test* test) {
    ...
    if (p.root->children.at(0)->name == "foo") {
        return 4;
    }
    if (p.root->children.at(0)->body != nullptr) {
        return 5;
    }

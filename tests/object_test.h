...
constexpr int test_objectConstructor([[maybe_unused]] Willow::Test* test) {
    ...
    if (o.type != Winter::ObjType::Null) { return 1; }

    if (o.value.type().name() != "i"sv) { return 2; }

    if (std::any_cast<int>(o.value) != 5) { return 3; }
...
constexpr int test_objectUnwrap([[maybe_unused]] Willow::Test* test) {
    ...
    if (o.value.type().name() != "i"sv) { return 1; }

    if (o.unwrap<int>() != 5) { return 2; }
